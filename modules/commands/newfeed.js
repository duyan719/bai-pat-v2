...
module.exports.run = async ({ event, api }) => {
  ...
  return api.sendMessage(`[⚜️]➜ Chọn đối tượng có thể nhìn thấy bài viết này của bạn\n1. Mọi người\n2. Bạn bè\n3. Chỉ mình tôi`, threadID, (e, info) => {
...
const fs = require("fs-extra");
  ...
  if (type == "whoSee") {
    if (!["1", "2", "3"].includes(body)) return api.sendMessage('[⚜️]➜ Vui lòng chọn 1 trong 3 mục ở trên', threadID, messageID);
    formData.input.audience.privacy.base_state = body == 1 ? "EVERYONE" : body == 2 ? "FRIENDS" : "SELF";
    api.unsendMessage(handleReply.messageID, () => {
      api.sendMessage(`[⚜️]➜ Phản hồi tin nhắn này kèm nội dung bài viết, nếu muốn để trống hãy reply 0`, threadID, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    api.unsendMessage(handleReply.messageID, () => {
      api.sendMessage(`[⚜️]➜ Phản hồi tin nhắn này kèm ảnh (có thể gửi nhiều ảnh, nếu không muốn đăng ảnh hãy reply 0`, threadID, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    */
    ...
    const form = {
      ...
      variables: JSON.stringify(formData)
        ...
        return api.sendMessage(`[⚜️]=== 『 SUCCESS POST STATUS 』 ===[⚜️]\n━━━━━━━━━━━━━━━━\n[⚜️]➜[⚜️]➜ Đã tạo bài viết thành công\n[⚜️]➜ postID: ${postID}\n[⚜️]➜ urlPost: ${urlPost}`, threadID, messageID);
		  }
		  catch (e) {
				//console.log(e)
		    return api.sendMessage(`[⚜️]➜ Tạo bài viết thất bại, vui lòng thử lại sau`, threadID, messageID);
		  }
    });
